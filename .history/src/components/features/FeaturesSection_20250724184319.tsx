					...
					<div className="flex flex-wrap justify-center gap-8">
						{features.slice(0, 3).map((feature) => (
							<Card
								...
								title={feature.title}
								hoverable
								className="h-full min-h-[120px] md:min-h-[140px] flex flex-col justify-between w-full md:w-1/3 max-w-xs"
							>
								<div>
									<p className={`text-${feature.color} text-sm mb-2 font-semibold`}>{feature.description}</p>
									<ul className="text-xs list-none pl-0 space-y-1">
										{feature.details.map((item, i) => (
											<li key={i} className="flex items-start gap-2">
												{React.cloneElement(feature.icon, { className: `h-4 w-4 text-${feature.color} mt-0.5` })}
